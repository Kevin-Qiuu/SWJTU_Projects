...
class Ui_MainWindow(QWidget):
    ...
    def setupUi(self, MainWindow):
        ...
        self.button_save_base.clicked.connect(self.button_get_base_clicked)
    ...
    def button_get_base_clicked(self):
        # import pyrealsense2 as rs
        from PyQt5.QtWidgets import QFileDialog
        import os  # 导入os模块
        ...
        for i in range(self.camera_num):
            ...
            self.baseImg[i] = self.img_r[i]
            cv2.cvtColor(self.baseImg[i], cv2.COLOR_BGR2GRAY)
            self.baseImg[i] = cv2.GaussianBlur(self.baseImg[i], (21, 21), 0)  # 高斯滤波进行模糊处理（光照变化或摄像头噪声）
        ...
        for i in range(self.camera_num):
            cv2.imwrite(os.path.join((self.folder_path), "color",
                                     "camera" + str(i + 1) + "_base.png"),
                        saved_color_image[i])
    ...
    def diffPhoto(self):
        try:
            for i in range(self.camera_num):
                ...
                cv2.cvtColor(self.img1[i], cv2.COLOR_BGR2GRAY)  # RGB图转灰度图
                self.img1[i] = cv2.GaussianBlur(self.img1[i], (21, 21), 0)  # 高斯滤波进行模糊处理（光照变化或摄像头噪声）
                res = cv2.absdiff(self.img1[i],self.baseImg[i])
                _,res = cv2.threshold(res, self.spinBox_min_2.value(), 255, cv2.THRESH_TOZERO)
                res = res.astype(np.uint8)
                percentage = (np.count_nonzero(res) * 100) / res.size
                print("百分比为")
                print(percentage)
                if percentage < self.spinBox_min.value():
                    self.photo_allow[i] = 1
                if percentage >= self.spinBox.value() and self.photo_allow[i] == 1:
                    self.photo_allow[i] = 0
                    QtCore.QTimer.singleShot(self.doubleSpinBox_interval_2.value() * 1000, lambda:self.save_data_choose(i))
                    # self.save_data_signal.emit(i)
        except:
            # print('error')
            # traceback.print_exc()
    ...
    def button_diff_click(self):
        ...
        if self.camera_work == 0:
            ...
            return
        if not self.baseImg:
            QMessageBox.about(self.centralwidget, '警告', '请先确定基图')
            return
        if self.tag_diff_data == 0:
            ...
            self.photo_allow = {}
            for i in range(self.camera_num):
                self.photo_allow.setdefault(i, 1)
    ...
    def button_save_data_click(self):
        ...
        try:
            ...
            QMessageBox.about(self.centralwidget, '警告', '           内存不足，请重新选择路径                 ')
            self.error_signal.emit()
    ...
    def button_open_camera_click(self):
        ...
        self.reword = {}##每次开机都得重新把diff的奖励值清零
        for i in range(self.camera_num):
            self.reword.setdefault(i, 0)
